...
function showRoleConfirm(uname, newRole, cb) {
    ...
    document.getElementById("confirm-title").textContent   = isPromote
...
function buildTaskCard(t) {
    ...
    if (t.dateDeadline) {
        ...
        if (!t.completed && days !== null) {
            if (days < 0)      cls = "expired"  // Pasada la fecha
...
;(function initDatePicker() {
    ...
    btnPrev.addEventListener("click", () => {
        dpMonth--
        if (dpMonth < 0) { dpMonth = 11; dpYear-- }  // Si pasa de enero, retrocede al diciembre anterior
        renderDpGrid()
    })
    ...
    btnNext.addEventListener("click", () => {
        dpMonth++
        if (dpMonth > 11) { dpMonth = 0; dpYear++ }  // Si pasa de diciembre, avanza al enero siguiente
        renderDpGrid()
    })
