...
document.getElementById("btn-save-task").addEventListener("click", async () => {
  ...
  try {
    if (state.editingTaskId) {
      ...
      const updated = await PATCH(`/projects/${state.currentProject.id}/tasks/${state.editingTaskId}`, { title, description: desc, dateDeadline: deadline, assignedToId: assignId })
      const idx = state.tasks.findIndex((t) => t.id === state.editingTaskId)
      if (idx > -1) state.tasks[idx] = updated
      toast("Tarea actualizada ✓")
    } else {
      const created = await POST(`/projects/${state.currentProject.id}/tasks`, { title, description: desc, dateDeadline: deadline, assignedToId })
